...
class CarMakerEnv(gym.Env):
    def __init__(self, host='127.0.0.1', port=10001, check=2, matlab_path='C:\CM_Projects\han_230706\src_cm4sl', simul_path='pythonCtrl_env21'):
        ...
        self.traj_data = pd.read_csv("datasets_traj_UTurn_env21_1.csv").loc[:, ["traj_tx", "traj_ty"]].values
    ...
    def step(self, action1):
        ...
        if state == False:
            ...
            state = np.concatenate((np.array([car_steer[0], car_v]), car_dev, lookahead_traj_rel)) #4 + 20

        # 리워드 계산
        reward_state = np.concatenate((car_dev, np.array([car_alHori, car_pos[0], car_v])))
    ...
    def getReward(self, state, time):
        ...
        if carx <= 100:
            reward_alHori = alHori * 2000


        e = - reward_devDist - reward_devAng - reward_alHori

        if self.test_num % 300 == 0 and self.check == 0:
            print("[Time: {}], [tx : {}], [Reward : {}], [Dist : {}]".format(time, carx, e, round(reward_devDist,2)))
