...
def load_data(prefix, road_type):
    data = {}
    data['info'] = pd.read_csv(f'{prefix}_{road_type}_info.csv')
    data['reward'] = pd.read_csv(f'{prefix}_{road_type}_reward.csv').loc[:, "0"].values
    if prefix != 'datasets_traj':
        data['action'] = pd.read_csv(f'{prefix}_{road_type}_action.csv')
    return data
...
#수정하는 부분
road_types = 'SLALOM_30m'
df_traj = pd.read_csv('datasets_traj_SLALOM_env17.csv')
...
def calc_performance(dataset, data_dict):
    ...
    initial_carv = get_value_or_interpolate(data_dict['carx'], data_dict['carv'], 35)
    escape_carv = get_value_or_interpolate(data_dict['carx'], data_dict['carv'], 335)
...
cones = [[50 + 30 * 0, -3, 11], [50 + 30 * 1, -3, 12],
         [50 + 30 * 2, -3, 11], [50 + 30 * 3, -3, 12],
         [50 + 30 * 4, -3, 11], [50 + 30 * 5, -3, 12],
         [50 + 30 * 6, -3, 11], [50 + 30 * 7, -3, 12],
         [50 + 30 * 8, -3, 11], [50 + 30 * 9, -3, 12],
         ]
car_width = 1.568
car_length = 4.3
